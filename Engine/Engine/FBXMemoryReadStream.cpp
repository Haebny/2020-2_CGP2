...
FBXMemoryReadStream::FBXMemoryReadStream(int size, fbxsdk::FbxManager* fbxManager) :
    m_streamData(nullptr), m_streamDataSize(size), m_streamPosition(0),
    m_state (EState::eEmpty), m_readerId(fbxManager->GetIOPluginRegistry()->FindReaderIDByExtension("fbx"))
{
}
...
void FBXMemoryReadStream::Seek(const FbxInt64& pOffset, const FbxFile::ESeekPos& pSeekPos)
{
